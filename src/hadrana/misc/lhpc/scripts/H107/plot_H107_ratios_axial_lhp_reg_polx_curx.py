...
ipol_X = 0
jcur_x = 0
qidx = 0  # try 0 and 1 etc
for t in [10,12,14]:
    A = np.asarray(R[t][source_set])
    for src_id in [0,1]:
        y = A[:, ipol_X, jcur_x, src_id, qidx, :]  # (Nbst, Ntau)
        mx = np.max(np.abs(y))
        print(f"t={t} src={src_id} qidx={qidx} max|y|={mx} mean|y|={np.mean(np.abs(y))}")
